...
def depth_colorize_with_mask(depthlist, background=(0,0,0), dmindmax=None):
    ...
    if dmindmax is None:
        ...
        # print(f"Valid depth min: {dmin}, max: {dmax}")
    else:
        dmin, dmax = dmindmax
        # print(f"Using provided depth range: min: {dmin}, max: {dmax}")
    
    norm_dth = np.ones_like(depthlist)*dmax # [B, H, W]
    norm_dth[batch, vx, vy] = (depthlist[batch, vx, vy]-dmin)/(dmax-dmin)
    # print(f"Normalized depth values range: min={norm_dth.min()}, max={norm_dth.max()}")
