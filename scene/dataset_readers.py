...
def calculate_confidence(depthmap, image, window_size=5):
    ...
    texture_confidence = (texture_confidence - texture_confidence.min()) / (texture_confidence.max() - texture_confidence.min())
    gradient_confidence = (gradient_confidence - gradient_confidence.min()) / (gradient_confidence.max() - gradient_confidence.min())
    ...
    final_confidence_map = (final_confidence_map - final_confidence_map.min()) / (final_confidence_map.max() - final_confidence_map.min())
...
def readColmapCameras(
    ...
    if confidence_map_output:
        confidence_map_dir = os.path.join(dataset_base_path, dataset_name, "confidence_map")
        confidence_vis_dir = os.path.join(dataset_base_path, dataset_name, "confidence_map_vis")
        os.makedirs(confidence_map_dir, exist_ok=True)
        os.makedirs(confidence_vis_dir, exist_ok=True)
    ...
    for idx, key in enumerate(sorted(cam_extrinsics,key=lambda x:cam_extrinsics[x].name)):
        ...
        resized_height = height // resolution
        resized_width = width // resolution
...
def optimize_depth(source, target, mask, depth_weight, prune_ratio=0.001):
    ...
    # print(f"Target depth range: min={target_min.item()}, max={target_max.item()}")
    ...
    # refined_min, refined_max = refined_source.min(), refined_source.max()
    # print(f"Optimized depth range: min={refined_min}, max={refined_max}")
