...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, usedepth=False, usedepthReg=False):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration % 1000 == 0:
            # if not (usedepth and iteration >= 2000): # avoid increasing the SH degree to ensure compatibility with depth regularization
        ...
        depthloss_align = viewpoint_cam.original_depthloss 
        # if depthloss_align is not None:
        #     print(f"Depth loss for current viewpoint: {depthloss_align}")
        # else:
        #     print("Depth loss not available for current viewpoint.")
        ...
        composite_image_loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image))
        loss = composite_image_loss
        ...
        if usedepth and viewpoint_cam.original_depth is not None:
            ...
            k_scale = 150
            depthloss_align_tensor = torch.tensor(depthloss_align)
            scale_factor = 0.6 * torch.exp(-k_scale * depthloss_align_tensor)

            current_depth_weight = scale_factor

            pixelwise_l1_loss = torch.abs(gt_maskeddepth - depth * depth_mask)

            if hasattr(dataset, 'confidence_map_output') and dataset.confidence_map_output:
                confidence_map_folder = os.path.join(dataset.source_path, "confidence_map")
                confidence_map_path = os.path.join(confidence_map_folder, f"{viewpoint_cam.image_name}_confidence.npy")
                
                if os.path.exists(confidence_map_path):
                    confidence_map = torch.from_numpy(np.load(confidence_map_path)).cuda()
                    weighted_loss = pixelwise_l1_loss * confidence_map
                else:
                    print(f"Warning: Confidence map not found at {confidence_map_path}. Using unweighted loss.")
                    weighted_loss = pixelwise_l1_loss
            else:
                weighted_loss = pixelwise_l1_loss
                
            deploss = weighted_loss.mean() * scale_factor
            loss = loss + deploss

        # Depth regularization loss (canny) from DRGS
        ...
        with torch.no_grad():
            ...
            ema_imageloss_for_log = 0.1 * composite_image_loss.item() + 0.9 * ema_imageloss_for_log

            # smooth the depth loss over iterations
            ema_depthloss_for_log = 0.1 * deploss.item() + 0.9 * ema_depthloss_for_log

            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}", "Deploss": f"{ema_depthloss_for_log:.4f}", "Image_loss": f"{ema_imageloss_for_log:.4f}", "Weight": f"{current_depth_weight:.4f}", "#pts": gaussians._xyz.shape[0]})
                progress_bar.update(10)
                if tb_writer is not None:
                    tb_writer.add_scalar('training_ema/depth_weight', current_depth_weight, iteration)
                    tb_writer.add_scalar('training_ema/depth_loss_ema', ema_depthloss_for_log, iteration)
                    tb_writer.add_scalar('training_ema/image_loss_ema', ema_imageloss_for_log, iteration)
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, deploss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, 
                            (pipe, background), txt_path = os.path.join(args.model_path, "metric.txt") )
            ...
            # Densification
            if iteration < opt.densify_until_iter and ((not usedepth) or gaussians._xyz.shape[0] <= 1500000): # Only densify if not using depth or fewer than 1.5M points
                ...
                # Reset opacity if depth is not being used
                if not usedepth:
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, deploss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, txt_path=None):
    if tb_writer:
        ...
        tb_writer.add_scalar('train_loss_patches/depth_loss', deploss.item(), iteration)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    # [Taekkii-HARDCODING] Save images (from DRGS)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[2_000, 7_000, 10_000, 15_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[2_000, 7_000, 10_000, 15_000, 30_000])
